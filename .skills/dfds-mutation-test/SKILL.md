...
- **File or directory**: Find source files, skip test files and generated code.
...
2. **Run tests** using the project's test runner:
   - **C#**: `dotnet test <project>` (optionally with `--filter`)
   - **TypeScript/JavaScript**: `npm test -- --run <pattern>` (or `npx jest`, `npx vitest`, etc.)
   - **Python**: `pytest <path>` (or `python -m unittest`)
   - **Go**: `go test ./<package>/...`
   - **Other**: Detect from project files (`Makefile`, `package.json`, `*.csproj`, `pyproject.toml`, etc.) or ask the user.
